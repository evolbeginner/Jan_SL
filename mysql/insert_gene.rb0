...
begin
  ...
  genes.each_key do |gene|
    ...
    if not parent_info.include?(gene)
      con.query("INSERT INTO Genes(id, orgn, exon_num, chr, start, end, pep_seq, up300_seq) \ 
                VALUES(\"#{gene}\", \"#{orgn}\", #{exon_num[gene]}, \"#{gff_info[gene]['chr']}\", #{gff_info[gene]['start']}, #{gff_info[gene]['end']}, \"#{pep_seq_info[gene]}\", \"#{up300_seq_info[gene]}\")"
               )
    else
      p gene, orgn, parent_info[gene], exon_num[gene]
      parent = parent_info[gene]
      con.query("INSERT INTO Genes(id, orgn, parent, exon_num, chr, start, end, pep_seq, up300_seq) \
                VALUES(\"#{gene}\", \"#{orgn}\", \"#{parent}\", #{exon_num[gene]}, \"#{gff_info[gene]['chr']}\", #{gff_info[gene]['start']}, #{gff_info[gene]['end']}, \"#{pep_seq_info[gene]}\", \"#{up300_seq_info[gene]}\")"
               )
    end
  end
